...
const promptLogOutCurrentlyLoggedInUser = () => {
    if (user?.loggedIn) {
      const confirmed = confirm(`Log out ${user.fname}?`);
      if (confirmed) {
        localStorage.removeItem('user');
        Toaster(`${user.fname} has been logged out successfully.`);
        // Optional: redirect or reload after logout
        // window.location.reload();
        // or navigate('/login');
      } else {
        Toaster(`Logout cancelled.`)
        // AlertService.info(`Logout cancelled.`);
      }
    } else {
        Toaster('No user is currently logged in.')
            ...
            <div className="checkout">
                ...
                <Box sx={{ borderBottom: 1, borderColor: "divider" }}>
                    ...
                    aria-label="basic tabs example">
                        ...
                        <Tab label="New Customer" {...a11yProps(1)} onClick={() => promptLogOutCurrentlyLoggedInUser()}/>

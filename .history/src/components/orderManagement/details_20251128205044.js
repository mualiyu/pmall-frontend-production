...
const OrderDetails= () => {
    ...
    const order = location?.state?.order;
        ...
        const handleStatusChange = async (orderState, e) => {
          ...
          // 👉 Build request body
          const requestBody = {
            stockist_id: orderState?.stockist?.id,
            sale_id: orderState?.pivot?.sale_id,
            product_id: orderState?.pivot?.product_id
          };
  ...
  useEffect(() => {
    ...
    return ( 
        <section className=" w-full" style={{display:"block"}}>
            ...
            onChange=<select onChange={(e) => handleStatusChange(orderState, e)}>

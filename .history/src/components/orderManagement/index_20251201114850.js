...
const OrderManagement = () => {
  ...
  const handleOrderStatus = async (orderStatus, e) => {
    ...
    const requestBody = {
      ...
      sale_id: parseInt(orderStatus?.pivot?.sale_id),
      product_id: parseInt(orderStatus?.pivot?.product_id)
    };
  
    try {
      // order/{orderID}/status
      const response = await fetch(`${BASE_URL}/order/${order.id}/status`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json;charset=UTF-8",
          Accept: "application/json",
      ...
      <Box sx={{ width: "100%" }}>
        ...
        <TabPanel value={value} index={0}>
          <TableContainer component={Paper}>
            ...
            value={orderStatus}
            onChange={(e) => handleOrderStatus(ord, e)}

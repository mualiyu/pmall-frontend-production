...
const onChangeHandlerForReason = (e) => {
  ...
  const handleProductLiveStatus = async (product, e) => {
   ...
   if(selectedStatus === "2") {
      ...
      addReasonForProductReject(product, selectedStatus )
    }
// 0 = Pending/No Action    1 = Accept/Publish   2 = Decline/Reject    3 = Send For Approval
    let endPointToUse = selectedStatus == "1" ? "products/update-status/admin" : selectedStatus == "2" ? "products/decline-product" : selectedStatus == "3" ? "products/sendforapproval" : "";


    let verb = selectedStatus == "1" ? "POST" : selectedStatus == "2" ? "PUT" : selectedStatus == "3" ? "PUT" : "";
    try {
  ...
  const addReasonForProductReject = async (prod, stat) => {
    setLoading(true);
              ...
              {products?.map((product, index) => (                   
                <TableRow key={product?.id} className="capitalize">
                  ...
                  <TableCell onClick={() => navigate(`/app/products/details/${product?.id}`)}>
                    ...
                    : product?.status === '1' ? <span className="badge success">Published</span> :
                    product?.status === '2' ? <span className="badge error c-chalk">Rejected</span> :
                    <span className="badge accent c-chalk">Sent For Approval</span>
                    }
                  </TableCell>
                  <TableCell  
                  onClick={() => {

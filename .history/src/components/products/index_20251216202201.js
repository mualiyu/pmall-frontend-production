...
const handleProductLiveStatus = async (product, e) => {
 ...
 function editBrand(data){
  ...
  const getAllRejectedProducts = async ()=> {
    try {
      const response = await fetch(`${BASE_URL}/product-brand/update?brand_id=${inputValues.id}&brand_image=${inputValues.brand_image}&name=${inputValues.name}&description=${inputValues.description}`, {
        method: 'POST',
        headers:{ 
          'Content-Type': 'application/json;charset=UTF-8', 
          "Accept": "application/json" ,

...
const ProductList = () => {
  ...
  const [rejectedProducts, setRejectedProduct] = useState(null);
...
const handleProductLiveStatus = async (product, e) => {
 ...
 function editBrand(data){
  ...
  const getAllRejectedProducts = async ()=> {
    try {
      const response = await fetch(`${BASE_URL}/products/all-decline-product`, {
        method: 'GET',
        headers:{ 
          'Content-Type': 'application/json;charset=UTF-8', 
          "Accept": "application/json" ,
      ...
      if (response.ok) {
        const data = await response.json();
        console.log('product:', data);
        setRejectedProduct(data)
      } else {
        const error = await response.text();
        setToastMsg('Error fetching product:', error)
        setToastType("error")
        setInterval(() => {
          setToastMsg("");
 }, 3000);

...
const columns = [
  ...
  { id: "cost_price", label: "Cost " },
  { id: "selling_price", label: "Selling " },
  { id: "created_at,", label: "Published" },
...
const ProductList = () => {
  ...
  const [rejectionMeta, setRejectionMeta] = useState({
    productId: null,
    status: null,
  });
...
const [statusFilter, setStatusFilter] = useState("all");
  ...
  const  [newRejectedProductModal, setNewRejectedProductModal] = useState(false);
  const  [newSubCategoryModal, setNewSubCategoryModal] = useState(false);
  const  [newBrandModal, setNewBrandModal] = useState(false);
  const [rejectedProducts, setRejectedProduct] = useState([]);
  ...
  const [productLiveState, setProductLiveState] = useState("");
  const handleModalClose = () => setNewProductModal(false);
  const handleCategoryModalClose = () => setNewCategoryModal(false);
  const handleRejectedProductModalClose = () => setNewRejectedProductModal(false);
...
const onChangeHandlerForReason = (e) => {
  setFormData({ ...formData, [e.target.name]: e.target.value });
};


const handleProductLiveStatus = async (product, e) => {
  ...
  if (selectedStatus === "2") {
    ...
    setNewRejectedProductModal(true);
    setLoading(false);
    return;
  }
  ...
  try {
    const response = await fetch(`${BASE_URL}/${endPointToUse}`, {
      method: verb,
      headers: {
        "Content-Type": "application/json;charset=UTF-8",
        Accept: "application/json",
  ...
  const addReasonForProductReject = async () => {
    setLoading(true);
  
    try {
      const response = await fetch(`${BASE_URL}/products/decline-product`, {
        method: "PUT",
        headers: {
        ...
        body: JSON.stringify({
          product_id: rejectionMeta.productId,
        status: rejectionMeta.status,
          reason: formData.rejectedReason,
        }),
      });
      ...
      handleRejectedProductModalClose();
      getProducts();
  
      setFormData({ rejectedReason: "" });
    } catch (error) {
      setToast({ message: "Failed to submit!", type: "error" });
    } finally {
      setLoading(false);
 ...
 function editBrand(data){
  ...
  const getProducts = () => {
    ...
    if (user?.accountType === "Admin") {
      ...
      url = `${BASE_URL}/products/?store_id=${id}`;
    }
    ...
    fetch(url, {
      ...
      .then((result) => {
        ...
        // Count published products (status === 1)
  ...
  const getAllRejectedProducts = () => {
    try {
      fetch(`${BASE_URL}/products/all-decline-product`, {
        method: 'GET',
        headers:{ 
          'Content-Type': 'application/json;charset=UTF-8', 
          "Accept": "application/json" ,
      ...
      .then((result) => {
        console.log(result)
        if(result?.status){
          setRejectedProduct(result?.data)
        }else{
          setToastMsg("Oops! there seems to be an error. Fill in correct credentials")
          setToastType("error")
          setInterval(() => {
...
console.log(user?.accountType)
  }, [newProduct, searchTerm, statusFilter, pageSize]);
        ...
        <div className="stat m-10">
          <div className="left__stat py-32">
            ...
            <h3 className="stat__value ml-10">
              ...
              {user?.accountType === "Admin" && (
              <p className="sub__title">Awaiting Approval</p> 
              )}
            </h3>
          </div>
          <div className="right__stat">
            ...
            <div className="right__sub">
              <h3 className="stat__value c-error">{rejectedProducts?.length}</h3>
              <p className="sub__title">Rejected</p>
            </div>
          </div>
        </div>
      </section>
            ...
            <Tabs
              ...
              aria-label="basic tabs example">
                  <Tab label="Products List" {...a11yProps(0)} />
                    ...
                    <Tab label="Rejected List" {...a11yProps(4)} />
            </Tabs>
      </Box>
...
{/* Ends Here */}
          <div className="">
            ...
            <select className="search__bar w-200" defaultValue={"default"} >
              ...
              <option value="Status 1"> All Products</option>
              <option value="Status 2"> Approved Products</option>
              <option value="Status 3"> Pending Products</option>
              <option value="Status 4"> Rejected Products</option>
            </select>
          </div>
          <div className="">
            <button
                ...
                <TableRow key={product?.id} className="capitalize">
                  {/* <TableCell className="b-r" onClick={() => navigate(`/app/products/details/${product?.id}`)}>
                    <div className="d-flex alc f-10 flex-start">
                        <img src={product.image} alt="" className="w50" alt={product?.name}/>
                    </div>
                  </TableCell> */}
                  ...
                  <TableCell onClick={() => navigate(`/app/products/details/${product?.id}`)}>
                    <p className="">{categories?.map(category =>(
                    category?.id == product?.category_id && category?.name 
                  ))}        </p>
                  </TableCell>
                  <TableCell onClick={() => navigate(`/app/products/details/${product?.id}`)}>
                  <p className="">{brands?.map(brand =>(
                      brand?.id == product?.brand_id && brand?.name 
                    ))} </p>
                  </TableCell>
                  <TableCell onClick={() => navigate(`/app/products/details/${product?.id}`)}> 
                  <p className=""> &#x20A6;{addCommasToNumberString(product?.cost_price)}  </p>
                  </TableCell>
                  <TableCell onClick={() => navigate(`/app/products/details/${product?.id}`)}> <p className=""> &#x20A6;{addCommasToNumberString(product?.selling_price)} </p> </TableCell>
                  <TableCell onClick={() => navigate(`/app/products/details/${product?.id}`)}> <p className=""> {moment(product?.created_at).add(1, "years").calendar()} </p></TableCell>
                  <TableCell onClick={() => navigate(`/app/products/details/${product?.id}`)}>
                    {" "}
                    {/* 0 = Pending/No Action    1 = Accept/Publish   2 = Decline/Reject    3 = Send For Approval */}
                    {product?.status === '0' ?
                    <span className="badge warning c-chalk">Pending</span>
                    : product?.status === '1' ? <span className="badge success c-chalk">Published</span> :
                    product?.status === '2' ? <span className="badge error c-chalk">Rejected</span> :
                    <span className="badge awaiting__approval c-chalk">Awaiting  Approval</span>
                    }
                  </TableCell>
                  <TableCell  
                  onClick={() => {
...
>
                  ...
                  <TableCell onClick={() => console.log(product.id)}>
                  <select
                      name="update__order"
                      className="last-name form-control"
                      value={productLiveState}
                      onChange={(e) => handleProductLiveStatus(product, e)}
      ...
      <TabPanel value={value} index={4}>
        <TableContainer component={Paper}>
          <Table sx={{ minWidth: 650 }} size="small" aria-label="Product Table">
            ...
            <TableBody>
            {Array.isArray(rejectedProducts) && rejectedProducts?.map((product) => (          
                <TableRow key={product?.id} className="capitalize">
                  <TableCell onClick={() => navigate(`/app/products/details/${product?.id}`)}>
                    <div className="lheight13">
                      <p className="">{product?.name} </p>
                  ...
                  <TableCell onClick={() => navigate(`/app/products/details/${product?.id}`)}>
                    ...
                    : product?.status === '1' ? <span className="badge success c-chalk">Published</span> :
                    product?.status === '2' ? <span className="badge error c-chalk">Rejected</span> :
                    <span className="badge warning c-chalk">Awaiting  Approval</span>
                    }
                  </TableCell>
                  <TableCell  
                  onClick={() => {
            ...
            <TableBody>
              {categories && categories.map((category, index) => (                   
                ...
                {/* <TableCell className="b-r">
                  <div className="d-flex alc f-10 flex-start">
                      <img src={category.category_image} className="w50" alt={category?.name}/>
                  </div>
                </TableCell> */}
                <TableCell>
          ...
          <Table sx={{ minWidth: 650 }} size="small" aria-label="Porduct Table">
            ...
            <TableBody>
              {brands && brands.map((brand, index) => (                   
                ...
                {/* <TableCell className="b-r">
                  <div className="d-flex alc f-10 flex-start">
                    <img src={brand.brand_image}  className="w50" alt={brand?.name}/>
                  </div>
                </TableCell> */}
                <TableCell>
      ...
      <Modal
        ...
        <Box sx={style}>
          ...
          <section className="flex__normal">
            <div className="w-250">
              <div className="">
                {inputValues.image || selectedProduct?.image ?
                 <img src={inputValues.image || selectedProduct?.image} className="w-100i" name="image" alt="Product" value={inputValues.image || selectedProduct?.image || ""} /> :
                <img src={profile || selectedProduct?.image} className="w-100i" name="image" value={inputValues.image || selectedProduct?.image || ""}  alt="User Profile"/>
                }
...
/>
            ...
            <form style={{ width: "100%" }}>
              <section className="flex-container mb-lg">
                ...
                <div className="pos-rel w100-m10 ">
                  ...
                  <select
                    ...
                    name="product_type"
                    value={selectedProduct?.product_type || inputValues.product_type || ""}
                    onChange={onChangeHandler}
                    >
              ...
              <section className="flex-container mb-lg">
                ...
                <div className="pos-rel w100-m10 ">
                  <label> Combo Price (lower than Selling price)</label>
                  <input
                    type="number"
                    className="form-control-input "
                    name="combo_price"
            ...
            id="tags-outlined"
            options={[]}
            value={Array.isArray(selectedProduct?.tags) 
              ? selectedProduct.tags 
              : Array.isArray(inputValues?.tags) 
              ? inputValues.tags 
              : []}
      ...
      <Modal
        ...
        <Box sx={style}>
          ...
          <section className="flex__normal">
            <div className="w-200">
              <div className="">
                <img src={profile} className="w-100i" name="image" value={inputValues.image|| ""} alt="Product" />
                <div className="pos-rel w100-m10 ">
                  <input
                    type="file"
                    className="form-control-input no-border"
      ...
      <Modal
        ...
        <Box sx={style}>
          ...
          <section className="flex__normal">
            <div className="w-200">
              <div className="">
                <img src={profile} className="profile_pic" name="image" value={inputValues.image|| ""} alt="Category" />
                <div className="pos-rel w100-m10 ">
                  <input
                    type="file"
                    className="form-control-input no-border"
...
{/* Reject Product Modal Reason */}

<Modal
        open={newRejectedProductModal}
        onClose={(reason) => {
          if (reason === 'backdropClick') return;
          handleRejectedProductModalClose();
        }}
        ...
        <Box sx={style}>
          <div className="mb-35">
            <Typography id="modal-modal-title">
              <h4 className="summary__title t-xl title-case">Rejected: Query Product</h4>
            </Typography>
            <div className="s-divider"></div>
          </div>
          <section className="flex__normal">
            ...
            <section className="flex-container flex-col g-20 mb-lg w-full">
              <form className="flex-container flex-col g-20 mb-lg">
                <div className="pos-rel w100-m10 ">
                  <label className="mb-7">Reason Why Product Was Rejected</label>
                  <textarea
                    className="search__bar w-100"
                    name="rejectedReason"
                  onChange={onChangeHandlerForReason}
                ...
                <div className="flex__normal w-full pull-right mt-35">
                  <button
                    onClick={handleRejectedProductModalClose}
                    className="btn btn-secondary p-25 pull-right mr-10">
                    Cancel
                  </button>
                    <button className="btn btn-primary p-25 pull-right" onClick={addReasonForProductReject} disabled={loading}
                  >
                    {loading ?<ButtonLoader /> : "Submit Comment"}
                  </button>
                </div>
              </form>
            </section>
...
{/* End Reject Product Modal Reason */}
      ...
      <Modal
        ...
        <Box sx={style}>
          ...
          <section className="flex__normal">
            <div className="w-200">
              <div className="">
                <img src={inputValues.category_image} className="profile_pic" name="image" value={inputValues.image|| ""} alt="Category" />
                <div className="pos-rel w100-m10 ">
                  <input
                    type="file"
                    className="form-control-input no-border"
      ...
      <Modal
        ...
        <Box sx={style}>
          ...
          <section className="flex__normal">
            <div className="w-200">
              <div className="">
                <img src={profile} className="profile_pic" name="image" value={inputValues.image|| ""} alt="Sub Category" />
                <div className="pos-rel w100-m10 ">
      ...
      <Modal
        ...
        <Box sx={style}>
          ...
          <section className="flex__normal">
            <div className="w-200">
              <div className="">
                <img src={profile} className="w-100i" name="image" value={inputValues.image|| ""} alt="Brand"/>
                <div className="pos-rel w100-m10 ">
                  <input
                    type="file"
                    className="form-control-input no-border"
      ...
      <Modal
        ...
        <Box sx={style}>
          ...
          <section className="flex__normal">
            <div className="w-200">
              <div className="">
                <img src={inputValues?.brand_image} className="w-100i" name="image" value={inputValues.image|| ""} alt="Brand" />
                <div className="pos-rel w100-m10 ">
                  <input
                    type="file"
                    className="form-control-input no-border"

...
const makeWithdrawal = async(e) => {
  ...
  try {
    const response = await fetch(
      `${BASE_URL}/withdrawal/request?category_image=${inputValues.category_image}&name=${inputValues.name}&description=${inputValues.description}`, {
      method: 'POST',
      headers:{ 
        'Content-Type': 'application/json;charset=UTF-8', 
        "Accept": "application/json" ,
    ...
    if (response.ok) {
      ...
      setToastMsg("Great! Request sent");

...
function handleWithdrawalAction(action, transactionId) {
  var approveUrl = 'https://yourapi.com/api/approve-withdrawal';
  var rejectUrl = 'https://yourapi.com/api/reject-withdrawal';

  var url = action === 'approve' ? approveUrl : rejectUrl;
  ...
  .then(function(result) {
    console.log('Withdrawal ' + action + 'd successfully:', result);
    // Optionally refresh table or update UI
  })
  .catch(function(error) {
    console.error('Error performing withdrawal action:', error);
  });
}
        ...
        <Table sx={{ minWidth: 650 }} size="small" aria-label="Vendors Table">
          ...
          <TableRow key={trx.id}>
              ...
              <div className="d-flex alc f-10 flex-start">
                  ...
                  <button
              className="btn btn-primary p-25"
              onClick={() => updateWithdrawal(true, trx.id)}
              >
              Approve 
            </button>
            <button
              className="btn btn-danger p-25"
              onClick={() => updateWithdrawal(false, trx.id)}
              >
              Decline
            </button>
                </div>

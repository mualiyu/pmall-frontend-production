...
const handleWithdrawalAction = (action, transactionId) => {
  let approveUrl = `${BASE_URL}/admin-withdrawal/approve/${transactionId}`;
  let rejectUrl = `${BASE_URL}/admin-withdrawal/reject/${transactionId}`;

  let url = action === 'approve' ? approveUrl : rejectUrl;
  ...
  .then(function(result) {
    setToast({ message: 'Withdrawal ' + action + 'd successfully!', type: action == 'approve' ? 'success' : 'error' });
  ...
  try {
    const response = await fetch(
      ...
      headers:{ 
        ...
        // body:JSON.stringify(inputValues, user.walletID)
    });
    // console.log(inputValues)
    if (response.ok) {
...
useEffect(()=> {
        ...
        <div className="flex-container alc">
          ...
          <div>
            <h3>Withdrawal History 
              <span className="badge danger">{withdrawals?.length}</span>
              </h3> 
        ...
        <Table sx={{ minWidth: 650 }} size="small" aria-label="Vendors Table">
          ...
          <TableRow key={trx.id}>
              ...
              <TableCell className="b-r">
                <div className="d-flex alc f-10 flex-start">
                  <div className="lheight13">
                    <h4 className="uppercase">{ trx.reference } </h4>
                  </div>
                </div>
              </TableCell>
              ...
              <div className="d-flex alc f-10 flex-start">
                  <div className="lheight13">
                    <h4 className="uppercase">
                    <PackageName id={trx?.user?.package_id } type={trx?.user.user_type} />
                      </h4>
                    </div>
              </div>
              </TableCell>
  ...
  color: withdrawal.status === 'Approved' ? 'green' :
         withdrawal.status === 'Declined' ? 'red' :
         'orange'
}}>
                 { trx.status }
                </h4>

...
const handleSubmit = async (e) => {
  ...
  const payload = {
      ...
      price: parseInt(inputValues.price, 10), // Convert to integer
      description: inputValues.description,
  };
console.log(payload);
  try {
      const response = await fetch(`${BASE_URL}/account-packages/create`, {
          method: "POST",
          headers: {
    ...
    {!loading && (<section>
      ...
      <section className="page__header">
        <div className="flex-container alc">
          <CardGiftcardIcon />
          <div>
          <h3>Create and Manage Account Packages</h3> 
          <p className="text-muted"> Admins can now create account registration packages for both Vendors and Affiliates</p>
          </div>
        </div>
      </section>
        ...
        <Box sx={style}>
          <div className="mb-35">
            <Typography id="modal-modal-title">
              <h4 className="summary__title t-xl title-case">Create New Package</h4>
            </Typography>
            <div className="s-divider"></div>
          </div>
